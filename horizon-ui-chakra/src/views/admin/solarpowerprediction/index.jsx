...
export default function SolarPowerPrediction() {
  ...
  useEffect(() => {
    const getSolarPrediction = async () => {
      try {
        console.log('Fetching solar prediction data...');
        const filterCriteria = formatDate(selectedDate);
        const response = await axios.post(`${API_URL}/api/getgeneration`, JSON.stringify({ filterCriteria }), {
          headers: {
        ...
        console.error('Error fetching solar prediction data:', error);
      }
    };
    getSolarPrediction();
  }, [selectedDate]);
    ...
    <Box pt={{ base: "180px", md: "80px", xl: "80px" }}>
      ...
      <Grid
        ...
        <Flex
          ...
          {averageActualPower !== null && maximumActualPower !== null && totalSolarPowerGenerated !== null && (
            <SimpleGrid
              columns={{ base: 1, md: 3 }}
              gap='20px'
              mb={{ base: "20px", xl: "0px" }}>
              ...
              {maximumActualPower !== null && (
                ...
                <MiniStatistics
                  name='Maximum Actual Power'
                  value={`${totalSolarPowerGenerated.toFixed(2)} kW`}
                /> 
              </>
              )} 
            </SimpleGrid>
      ...
      <SimpleGrid columns={{ base: 1, md: 2, xl: 2 }} gap='20px' mb='20px'>
        <h1 className="chart-heading">Solar Power Prediction Chart</h1>
        <SimpleGrid columns={{ base: 1, md: 1, xl: 1 }} gap='20px'>
          <DatePicker selected={selectedDate} onChange={handleDateChange} dateFormat="dd-MM-yyyy" />
        </SimpleGrid>        
      </SimpleGrid>
      <ResponsiveContainer width="100%" aspect={3}>
        <LineChart data={solarData} width={500} height={300} margin={{ top: 5, right: 100, left: 30, bottom: 5 }}>
          <CartesianGrid />
